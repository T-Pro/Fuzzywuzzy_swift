...
class StringFuzzywuzzyExtensionTests: XCTestCase {
  ...
  func testTokenSetRatio_basicCases() {
    ...
    // Duplicated tokens
    XCTAssertEqual(String.fuzzTokenSetRatio(lhs: "fuzzy fuzzy wuzzy was a bear", rhs: "wuzzy fuzzy was a bear"), 100)
    XCTAssertEqual(String.fuzzTokenSetRatio(lhs: "fuzzy$*#&)$#(wuzzy*@()#*()!<><>was a bear", rhs: "wuzzy wuzzy fuzzy was a bear"), 100)
    // Different tokens
  ...
  func testTokenSortRatio_basicCases() {
    ...
    // Token order
    XCTAssertEqual(String.fuzzTokenSortRatio(lhs: "fuzzy wuzzy was a bear", rhs: "wuzzy fuzzy was a bear"), 100)
    XCTAssertEqual(String.fuzzTokenSortRatio(lhs: "fuzzy$*#&)$#(wuzzy*@()#*()!<><>was a bear", rhs: "wuzzy fuzzy was a bear"), 100)
    // Different tokens
    XCTAssertEqual(String.fuzzTokenSortRatio(lhs: "cat dog", rhs: "dog mouse"), 67)
  }
  ...
  func testPartialRatio_variousCases() {
    ...
    XCTAssertEqual(String.fuzzPartialRatio(lhs: "", rhs: ""), 100)

    // Substring match
    XCTAssertEqual(String.fuzzPartialRatio(lhs: "abcd", rhs: "XXXbcdeEEE"), 75)
    XCTAssertEqual(String.fuzzPartialRatio(lhs: "what a wonderful 世界", rhs: "wonderful 世"), 100)
    XCTAssertEqual(String.fuzzPartialRatio(lhs: "this is a test", rhs: "this is a test!"), 93)
  }
  ...
  func testCommonSubstrings_pairs() {
    ...
    let pairs1: [CommonSubstringPair] = CommonSubstrings.pairs(lhs: "aaabbcde", rhs: "abbdbcdaabde")
    XCTAssertTrue(pairs1.contains { $0.len >= 2 }) // Should find at least one common substring of length >= 2

    // Identical strings
    let pairs2: [CommonSubstringPair] = CommonSubstrings.pairs(lhs: "abcdef", rhs: "abcdef")
  ...
  func testStringMatcher_ratioRange() {
    ...
    for (lhs, rhs) in strPairs {
      ...
      XCTAssert(ratio <= 1 && ratio >= 0, "Ratio out of bounds for '\(lhs)' and '\(rhs)': \(ratio)")
    }
  }

  func testTokenSetRatio_realExamples() {
    // Examples from README
    XCTAssertEqual(String.fuzzTokenSetRatio(lhs: "fuzzy was a bear", rhs: "fuzzy fuzzy was a bear"), 100)
    XCTAssertEqual(String.fuzzTokenSortRatio(lhs: "fuzzy was a bear", rhs: "fuzzy fuzzy was a bear"), 84)
  }

  func testTokenSortRatio_fullProcessToggle() {
    // With fullProcess = false, special characters are not removed
    let ratioWithProcess: Int = String.fuzzTokenSortRatio(lhs: "fuzzy+wuzzy(was) a bear", rhs: "wuzzy fuzzy was a bear")
    let ratioWithoutProcess: Int = String.fuzzTokenSortRatio(lhs: "fuzzy+wuzzy(was) a bear", rhs: "wuzzy fuzzy was a bear", fullProcess: false)
  ...
  func testFuzzRatio_identicalAndDifferent() {
    ...
    XCTAssertEqual(String.fuzzRatio(lhs: "abc", rhs: "def"), 0)
